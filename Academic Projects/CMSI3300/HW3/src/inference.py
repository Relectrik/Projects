    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for position in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                nextBeliefs[newPos] += prob * self.beliefs[position]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            if emissionModel[trueDistance] > 0:
                weighted[particle] += emissionModel[trueDistance]
        ...
        if weighted.totalCount() == 0:
            ...
            self.particles = util.nSample(beliefs.values(), beliefs.keys(), self.numParticles)
        else:
            self.particles = util.nSample(weighted.values(), weighted.keys(), self.numParticles)    
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            if particle not in newParticleDists:
                newParticleDists[particle] = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
        
        self.particles = [util.sample(newParticleDists[particle]) for particle in self.particles]
