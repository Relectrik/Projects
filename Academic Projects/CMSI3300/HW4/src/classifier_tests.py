...
TOX_TEXT_TRAIN, TOX_TEXT_TEST, TOX_LABELS_TRAIN, TOX_LABELS_TEST = train_test_split(
    TOXICITY_DATA["comment"], TOXICITY_DATA["toxic"], test_size=0.3, random_state=1337
)
...
SAL_X_TRAIN, SAL_X_TEST, SAL_Y_TRAIN, SAL_Y_TEST = train_test_split(
...
class ClassiferTests(unittest.TestCase):
    ...
    # -------------------------------------------------------------------------------------------------
    def verify_data(
        self, X: pd.DataFrame, Y: pd.DataFrame, x_len: int, y_len: int
    ) -> None:
        DAT_ERR = (
            "[X] Make sure your constructor isn't tampering with the data frame itself!"
        )
    ...
    def test_toxicity_filter_vectorizer(self) -> None:
        self.verify_data(
            TOX_TEXT_TRAIN, TOX_LABELS_TRAIN, TOX_TEXT_TRAIN_SIZE, TOX_LABEL_TRAIN_SIZE
        )
        talk_xic = ToxicityFilter(TOX_TEXT_TRAIN, TOX_LABELS_TRAIN)
        self.assertTrue(
            talk_xic.vectorizer is not None,
            "[X] Make sure your vectorizer is initialized in the constructor",
        )
        self.assertTrue(
            hasattr(talk_xic.vectorizer, "vocabulary_"),
            "[X] Make sure your vectorizer is .fit on the text_train parameter in your constructor",
        )
        ...
        if VERBOSE:
            ...
            print(
                "  > Vectorizer Vocabulary Sample (Randomish 30): "
                + str(list(vocab.items())[0:30])
            )
    ...
    def test_toxicity_filter_classification(self) -> None:
        ...
        try:
            ...
            raise ValueError(
                "[X] Did you use the fit_transform method of your vectorizer in classify rather than just transform? (Should only be fit once in constructor)"
            )

        labels_assigned = [
            "TOXIC" if classes_assigned[i] else "Non-toxic"
            for i in range(len(classes_assigned))
        ]
