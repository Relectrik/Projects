...
class ToxicityFilter:
    ...
    def __init__(self, text_train: pd.DataFrame, labels_train: pd.DataFrame):
        ...
        self.NBC: "MultinomialNB" = MultinomialNB().fit(
            self.vectorizer.transform(text_train), labels_train
        )
    ...
    def classify(self, text_test: list[str]) -> list[int]:
        ...
        """
        return self.NBC.predict(self.vectorizer.transform(text_test))

    def test_model(
        self, text_test: pd.DataFrame, labels_test: pd.DataFrame
    ) -> tuple[str, dict]:
        """
        ...
        return (
            classification_report(labels_test, prediction, output_dict=False),
