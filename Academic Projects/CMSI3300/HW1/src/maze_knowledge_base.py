...
class MazeKnowledgeBase:
    ...
    def ask(self, query: "MazeClause") -> bool:
        ...
        working_clauses: set["MazeClause"] = deepcopy(self.clauses)
        for alpha_prop in query.props.items():
            working_clauses.add(MazeClause([(alpha_prop[0], not alpha_prop[1])]))

        new_clauses: set["MazeClause"] = set()

        while True:
            for clause_1, clause_2 in itertools.combinations(working_clauses, 2):
                resolvents: set["MazeClause"] = MazeClause.resolve(clause_1, clause_2)
    ...
    def simplify_self(
        ...
        """
        self.clauses = MazeKnowledgeBase.simplify_from_known_locs(
    ...
    def simplify_from_known_locs(
        ...
        for loc in known_pits | known_safe:
            clauses = MazeKnowledgeBase.get_simplified_clauses(
    ...
    def get_simplified_clauses(
        clauses: set["MazeClause"], loc: tuple[int, int], is_pit: bool
    ) -> set["MazeClause"]:
        """
