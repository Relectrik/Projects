...
class Environment:
    ...
    def __init__(
        self, maze: list[str], tick_length: int = 1, verbose: bool = True
    ) -> None:
        """
    ...
    def get_cardinal_locs(
        self, loc: tuple[int, int], offset: int
    ) -> set[tuple[int, int]]:
        """
        ...
        return set(
            filter(
                lambda loc: loc[0] >= 0
                and loc[1] >= 0
                and loc[0] < self._cols
                and loc[1] < self._rows
                and loc in self._playable,
                pos_locs,
            )
        )
    ...
    def _get_wrn_num(self, loc: tuple[int, int]) -> int:
        ...
        return len(
            {
                adj_tile
                for adj_tile in self.get_cardinal_locs(loc, 1)
                if adj_tile in self._pits
            }
        )
    ...
    def _make_agent_maze(self) -> list:
        ...
        sub_regexp = (
            "["
            + Constants.PIT_BLOCK
            + Constants.SAFE_BLOCK
            + "".join(Constants.WRN_BLOCKS)
            + "]"
        )
    ...
    def _run_one_tick(self) -> tuple[tuple[int, int], int]:
        ...
        if not self._test_move_request(next_loc):
            if self._verbose:
                print(
                    "\n [X] Provided an invalid move request ("
                    + str(next_loc)
                    + "); must choose from locations along the frontier."
                )
        ...
        penalty = dist + (
            Constants.get_pit_penalty() if self._pit_test(self._player_loc) else 0
        )
...
if __name__ == "__main__":
    ...
    env = Environment(mazes[5], tick_length=1, verbose=True)
    env.start_mission()
