...
class Environment:
    ...
    def __init__(self, maze, verbose=False, debug=False, step=False, gui=True):
        ...
        if not Environment.running_agent:
            Environment.running_agent = PacmanAgent(maze)
    ...
    def move(self):
        ...
        if self._moves > Constants.MAX_MOVES:
            ...
            Environment.running_agent.give_transition(MazeProblem(state), self._last_act, MazeProblem(next_state), True)
            self._cleanup()
            return
        ...
        next_act = Environment.running_agent.choose_action(mp, dict(mp.legal_actions(self.get_player_loc())))
        self._last_act = next_act
        if self._debug:
            print("next_act: ", next_act)
        next_mp = self._move_request(next_act)
        ...
        if self._ghost_test(self._player_loc):
            ...
            next_mp = MazeProblem(self._maze)
            Environment.running_agent.give_transition(mp, next_act, next_mp, True)
            self._cleanup()
            return
        ...
        # Transition recorded
        next_mp = MazeProblem(self._maze)
        Environment.running_agent.give_transition(mp, next_act, next_mp, False)
    ...
    def _cleanup(self):
        ...
        Environment.running_agent.give_terminal()
        if self._gui:
    ...
    def _move_request(self, move):
        ...
        new_mp = self._update_mazes(self._player_loc, new_loc)
    ...
    def _update_mazes(self, old_loc, new_loc):
        ...
        return MazeProblem(copy.deepcopy(self._maze))
    ...
    def run_game (verbose=False, debug=False, step=False, gui=True, maze = Constants.MAZE, agent = None):
        if not agent is None:
            Environment.running_agent = agent
        Environment.running_env = Environment(maze, verbose=verbose, debug=debug, step=step, gui=gui)
