...
class ReplayMemory(object):
    ...
    def __init__(self, capacity: int = Constants.MEM_SIZE):
        '''
    ...
    def load(self) -> None:
        ...
        if exists(Constants.MEM_PATH):
            ...
            print("[X] Warning: attempted to load Replay Memory from missing path (Constants.MEM_PATH); defaulting to blank memory.")
    ...
    def vectorize_maze(maze: list[str]) -> torch.tensor:
        '''
    ...
    def vectorize_move(move: str) -> torch.tensor:
        '''
...
class PacNet(nn.Module):
    ...
    def __init__(self, maze):
        ...
        :maze: The Pacman Maze structure on which this PacNet will be trained, usually
               Constants.MAZE
        """
    ...
    def save(self) -> None:
        """
        Saves the network's current parameters to the path in Constants.PARAM_PATH
        """
        torch.save(self.state_dict(), Constants.PARAM_PATH)
    
    def load(self) -> None:
        """
        If they exist, loads the network parameters from Constants.PARAM_PATH
        """
        if exists(Constants.PARAM_PATH):
            self.load_state_dict(torch.load(Constants.PARAM_PATH))
        else:
            print("[X] Warning: attempted to load PacNet parameters from missing parameters file (Constants.PARAM_PATH); using default starting weights instead.")
    
    def load_from_pacnet(self, other_net) -> None:
        """
        Copies the network parameters from the other PacNet input
        """
        self.load_state_dict(other_net.state_dict())

def run_training_loop(maze: list[str]=Constants.MAZE, n_games: int=Constants.N_SIMS, verbose=Constants.VERBOSE, gui=Constants.GUI, agent = None, sim_name="pacman_sim") -> dict:
    """
    Executes n games of Pacman within the given maze and tracks / plots
    """
    wins = 0
    win_ema = 0
    move_ema = 0
    ...
    for i_episode in range(n_games):
        print("==============================")
        print("Iteration " + str(i_episode+1) + " / " + str(n_games))
        print("==============================")
        outcome = environment.Environment.run_game(maze=maze, debug=Constants.DEBUG, verbose=verbose, step=False, gui=gui, agent=agent)
        wins += outcome["win"]
    ...
    if verbose:
        plt.show()
    plt.savefig("./plots/" + sim_name + ".png")
