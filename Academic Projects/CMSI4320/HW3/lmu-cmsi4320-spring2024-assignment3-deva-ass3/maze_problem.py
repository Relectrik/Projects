...
class MazeProblem:
        ...
        for (row_num, row) in enumerate(maze):
            for (col_num, cell) in enumerate(row):
                ...
                if cell == Constants.PLR_BLOCK:
                    self._player_loc = (col_num, row_num)
    ...
    def legal_actions(self, player_loc):
        """
        Returns all legal actions available to a maze agent from the given state
        :player_loc: The current position from which to obtain the legal actions
    ...
    def transitions(self, player_loc):
        """
        ...
        [("R", 1, (2, 1)), ("D", 1, (1, 2))]
        :player_loc: A maze location tuple
