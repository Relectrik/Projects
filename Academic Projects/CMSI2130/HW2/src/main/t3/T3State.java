...
public class T3State {
    ...
    public Map<T3Action,T3State> getTransitions () {
        //Instantiate data structure to return.
        TreeMap <T3Action, T3State> possibleMoves = new TreeMap <T3Action, T3State>();
       
        //Nested for loop to append row and column number to moves we can make, then
        //initializing a new T3Action using getNextState.
        for (int i = 0; i < getOpenTiles().size(); i++) {
            for (int j = 0; j < getMoves().length; j++) {
                int[] position = getOpenTiles().get(i);
                T3Action action = new T3Action(position[0], position[1], getMoves()[j]);
                possibleMoves.put(action, getNextState(action));
            }
        }
